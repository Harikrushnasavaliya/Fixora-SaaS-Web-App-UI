...
export async function createDemoPaymentIntent(req, res) {
    try {
        ...
        if (existing) {
            return res.json({
                message: "Demo payment intent ready",
...
export async function confirmDemoPayment(req, res) {
    try {
        ...
        if (!payment_id || !client_secret) return res.status(400).json({ message: "payment_id and client_secret are required" });
        if (!isValidObjectId(payment_id)) return res.status(400).json({ message: "Invalid payment_id" });

        const user = await User.findById(userId).select("_id role");
        if (!user || user.role !== "customer") return res.status(403).json({ message: "Only customers can confirm payment" });
        ...
        if (client_secret !== `demo_secret_${payment._id}`) return res.status(400).json({ message: "Invalid client_secret" });

        const shouldFail = String(simulate || "").toLowerCase() === "fail";
        if (shouldFail) {
            payment.status = "failed";
            await payment.save();
            await Booking.updateOne({ _id: payment.booking_id }, { $set: { payment_status: "failed" } });
            return res.status(400).json({ message: "Demo payment failed", payment });
        }

        payment.status = "paid";
        payment.transaction_ref = makeTxnRef();
        ...
        return res.json({ message: "Demo payment success", payment });
    } catch (e) {
        return res.status(500).json({ message: e.message });
    }
}
